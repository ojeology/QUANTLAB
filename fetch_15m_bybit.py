"""
Fetch 3-YEAR 15m from BYBIT (alternative to OKX-throttled / Binance-blocked) for the
core symbols, via aiohttp async. Bybit v5 spot klines: deep history, permissive.
Maps BTC_USDT_SWAP -> BTCUSDT. Resume-safe.
"""
...
BIN = {s: s.split("_")[0] + "USDT" for s in CORE}
...
async def fetch_sym(session, sem, sym, bin_sym):
    ...
    async with sem:
        ...
        for attempt in range(5):
            try:
                while start < END:
                    params = {"category":"spot","symbol":bin_sym,"interval":"15","start":start,"limit":1000}
                    async with session.get("https://api.bybit.com/v5/market/klines", params=params, timeout=30) as r:
                        if r.status == 429:
                            await asyncio.sleep(5); continue
                        j = await r.json()
                    if j.get("retCode") != 0 or "list" not in j.get("result",{}):
                        print(f"  {sym}: bybit err {j.get('retCode')}", flush=True); break
                    data = j["result"]["list"]
                    if not data: break
                    bars.extend(data)
                    start = int(data[-1][0]) + INT
        ...
        df = pd.DataFrame(bars, columns=["ts","open","high","low","close","vol","turnover"])
...
async def main():
    ...
    print(f"[done] Bybit 15m fetched {len(got)} core symbols", flush=True)


if __name__ == "__main__":
    print(f"[start] {len(CORE)} core symbols from Bybit (3yr 15m, async)", flush=True)
    t0 = time.time()
    asyncio.run(main())
