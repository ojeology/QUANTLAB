"""
TEST 26 — PORTFOLIO: MR champion (SVMQ65Adaptive, Family A) + condition-aware TREND
(RF-filtered 1H Donchian). They are complementary regimes (MR near-mean, trend
far-from-mean). Each validated 3-yr solo; combine 50/50, each risks 0.5% of total.
Goal: smoother equity than either solo, still PF>=1.3 each year. Carefully written
(defaultdict(list) in monthly agg). 1H, 20-sym, 2024-2026 (2023 fetched in-run).
"""
...
def report(name, mr_list, tr_list, risk_each=0.005):
    print("\n"+"="*82)
    print(name)
    print("="*82)
    for Y in [2024,2025,2026]:
        ...
        print(f"\n[{Y}] n={len(mrs)+len(trs)} win={wins:.0%} PF@c={pf:.3f} ret(1%)={(eqA/2+eqB/2-1):+.1%} MAX DD={ymdd:.1%} prof-months={pos}/{len(msum)}")
...
report("PORTFOLIO: MR champion + condition-aware TREND (50/50, 0.5% risk each)", mr_champ, tr_champ)
print("\n[done]")
