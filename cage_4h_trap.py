"""
TEST 22 — 4H-CAGE TRAP on 1H. Use the 4H candle's high/low as the "cage" (trap)
boundaries; on the 1H chart, when price TAGS the 4H low and bounces back (or tags the
4H high and rejects), catch the reversal. Mean-reversion at higher-TF support/resistance.
...
def backtest_4h_trap(df):
    """4H-cage trap: fade 4H-low on bounce (long), fade 4H-high on reject (short)."""
    df = df.copy()
    h4 = df.resample("4H").agg(high=("high","max"), low=("low","min")).shift(0)
    ...
    for i in range(4, len(df)):
        ...
        if not in_pos:
            # LONG: tagged 4H low and bounced (bullish reversal bar)
            if (l <= cl) and (c > cl) and (c > o) and a > 0:
                ep=c; stop=cl - a; target=(ch+cl)/2.0; side="L"; in_pos=True
            # SHORT: tagged 4H high and rejected (bearish reversal bar)
            elif (h >= ch) and (c < ch) and (c < o) and a > 0:
                ep=c; stop=ch + a; target=(ch+cl)/2.0; side="S"; in_pos=True
        else:
            ex=None; et=None
            if side=="L":
...
all_trades = []
for s in feats:
    for t in backtest_4h_trap(feats[s]):
        t["sym"]=s; all_trades.append(t)
all_trades.sort(key=lambda t: t["entry_time"])
for t in all_trades: t["adj_r"]=t["r"]-2*FEE
print(f"[bt] 4H-cage trap trades 2023-2026: {len(all_trades)}", flush=True)


def report():
    print("\n"+"="*80)
    print("TEST 22 — 4H-CAGE TRAP on 1H (fade 4H extremes on 1H reversal)")
    print("="*80)
